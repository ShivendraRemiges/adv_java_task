...
public class HelloController {
    ...
    public ResponseEntity<Object> sayHello() {
        try {
            ...
            return JasonResponse.generateResponse("success", "", "", responseData, reqId);
        } catch (Exception e) {
    ...
    @PostMapping("/hello")
    public ResponseEntity<Object> sayHello(@RequestBody JasonRequest request) {
        try {
    ...
    @PostMapping("/mysum")
    public ResponseEntity<Object> calculateSum(@RequestBody JasonRequest request) {
        try {
            ...
            switch (operation.toLowerCase()) {
                ...
                case "divide":
                    if (num2 == 0) {
                        return JasonResponse.generateResponse("error", "DIVIDE_BY_ZERO", "Cannot divide by zero",
                                null, "");
                    }
                    result = num1 / num2;
                    break;
                default:
                    return JasonResponse.generateResponse("error", "INVALID_OPERATION", "Invalid operation specified",
                            null, "");
            }
            ...
            return JasonResponse.generateResponse("error", "INVALID_NUMBER_FORMAT", "Invalid number format", null,
                    "");
        } catch (Exception e) {
    ...
    @PostMapping("/mycalc")
    public ResponseEntity<Object> performCalculation(@RequestBody JasonRequest request) {
        try {
            ...
            // Generating the custom response using ResponseHandler
            return JasonResponse.generateResponse("success", "", "", responseData, "afkjekfsdkf454354");
        } catch (Exception e) {
            // Handling exceptions
            return JasonResponse.generateResponse("error", "INTERNAL_SERVER_ERROR", e.getMessage(), null, "");
        }
